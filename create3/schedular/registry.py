...
def get_task_callback(task):
    """Return the callback function for a given task."""
    return TASK_CALLBACKS.get(task)

def check_requirements(scheduler: "TaskSchedular", task) -> bool:
    ...
    match task:
        ...
        case RemoteTasks.CONTROLLER:
            if not (scheduler._find_device(Nodes.CREATE3_ROBOT) and scheduler._find_device(Nodes.CREATE3_REMOTE)):
                scheduler.print_warning(f'{task} task requires the {Nodes.CREATE3_ROBOT} and {Nodes.CREATE3_REMOTE} nodes.')
                return False
