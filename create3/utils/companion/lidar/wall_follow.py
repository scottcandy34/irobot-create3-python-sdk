...
def pid_lidar_to_twist(lidar: Lidar) -> Twist:
    ...
    right_distances = [d  for d in lidar.get_right_slice() if 0.1 < d < lidar.range_max]
    front_distances = [d for d in lidar.get_front_slice() if 0.1 < d < lidar.range_max]

    wall_dist = sum(right_distances) / len(right_distances)
    ...
    twist_msg.linear.x = linear_output / 100
